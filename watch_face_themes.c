...
//place these in the temp.c file
extern float Temperature_C ;
extern float Temperature_F ;
extern float Temperature_K ;
extern char temperature_C_string[20];
extern char temperature_F_string[20];
extern char temperature_K_string[20];
/*============================================*/
#endif
...
//empty for now, but will move from main.
    ...
    {
    case 0:
        display_center("Sunday", 64, 100);
        break;
    case 1:
        display_center("Monday", 64, 100);
        break;
    case 2:
        display_center("Tuesday", 64, 100);
        break;
    case 3:
        display_center("Wednesday", 64, 100);
        break;
    case 4:
        display_center("Thursday", 64, 100);
        break;
    case 5:
        display_center("Friday", 64, 100);
        break;
    case 6:
        display_center("Saturday", 64, 100);
        break;
    default:
        display_center("Monday", 64, 100);
...
void large_TIME(  )
{
    if (change_watch_face == 1)
    {
        ...
        Graphics_clearDisplay(&g_sContext);
        change_watch_face = 0; //should be here, but we will change it below to get 2 font sizes.
    }
...
//    display_center(months_string, 20, 30);
    ...
    sprintf(hours_string, "%02X:", hour);
    display_center(hours_string, 50, 40);

    sprintf(minutes_string, "%02X", minute);
    display_center(minutes_string, 50, 85);
    ...
    if (change_watch_face == 1)
      {
          ...
          Graphics_setBackgroundColor(&g_sContext, GRAPHICS_COLOR_DARK_SLATE_BLUE);
...
//    sprintf(temperature_C_string, "%1.2f *C", Temperature_C);
//    display_center(temperature_C_string, 50, 40);

    sprintf(temperature_C_string, "%2.2f *C", Temperature_C);

//    sprintf(temperature_C_string, "%1.2f *C", Temperature_C);
    display_center(temperature_C_string, 50, 40);

    sprintf(temperature_F_string, "%2.2f  *F", Temperature_F);
    display_center(temperature_F_string, 50, 70);

    sprintf(temperature_K_string, " %2.2f  K", Temperature_K);
    display_center(temperature_K_string, 50, 100);


//    void put_on_screeen(char * format, uint8_t _x, uint8_t _y){
//            char _string[20];
//            sprintf(_string, format);
//            display_center(_string, _x, _y);
//    }
